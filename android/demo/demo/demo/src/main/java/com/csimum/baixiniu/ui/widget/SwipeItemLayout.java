...
public class SwipeItemLayout extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        //layout main
        LayoutParams mainItemViewLp = (LayoutParams) mainItemView.getLayoutParams();
        mainItemView.layout(
                pl+mainItemViewLp.leftMargin,
                pt+mainItemViewLp.topMargin,
                getWidth()-pr-mainItemViewLp.rightMargin ,
                getHeight()-pb-mainItemViewLp.bottomMargin);

        //layout menu
        int totalLength = 0;
        int menuLeft = mainItemView.getRight()+mainItemViewLp.rightMargin;
        for(int i=0;i<getChildCount();i++){
            View menuView = getChildAt(i);
            mainItemViewLp = (LayoutParams) menuView.getLayoutParams();

            if(mainItemViewLp.itemType==0x01)
                continue;

            int tempLeft = menuLeft+mainItemViewLp.leftMargin;
            int tempTop = pt+mainItemViewLp.topMargin;
            menuView.layout(
                    tempLeft,
                    tempTop,
                    tempLeft+menuView.getMeasuredWidth()+mainItemViewLp.rightMargin - mainItemViewLp.getMarginEnd(),
                    tempTop+menuView.getMeasuredHeight()+mainItemViewLp.bottomMargin);

            menuLeft = menuView.getRight()+mainItemViewLp.rightMargin;
            totalLength += mainItemViewLp.leftMargin+mainItemViewLp.rightMargin +  mainItemViewLp.getMarginEnd()+menuView.getMeasuredWidth();
        }

...
#define  LOGI(...)  __android_log_print(ANDROID_LOG_INFO,CAPTUREPALYER_LOG_TAG,__VA_ARGS__)
#define  LOGW(...)  __android_log_print(ANDROID_LOG_INFO,CAPTUREPALYER_LOG_TAG,__VA_ARGS__)
#define  LOGE(...)  __android_log_print(ANDROID_LOG_INFO,CAPTUREPALYER_LOG_TAG,__VA_ARGS__)
...
#define LOGI(...) ((void)av_log(NULL, AV_LOG_INFO,   __VA_ARGS__))
#define LOGW(...) ((void)av_log(NULL, AV_LOG_WARNING,  __VA_ARGS__))
#define LOGE(...) ((void)av_log(NULL, AV_LOG_ERROR,  __VA_ARGS__))
...
//    }
    ...
    av_image_copy(pic->data, pic->linesize, data, linesize, AV_PIX_FMT_YUV420P, frame->linesize[0],
                  height);
}
...
static inline const char *safe_str(const char *s)
{
    ...
    static inline int astrcmpi_n(const char *str1, const char *str2, size_t n)
    {
        if (!n)
            return 0;
        if (!str1)

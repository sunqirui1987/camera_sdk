 ...
 */
typedef struct re_pool{
	pthread_mutex_t lock;
	pthread_cond_t notify;
	pthread_t *threads;
	re_task_t *task_queue; // task queue
	int queue_size;
	int queue_count;
	int thread_count;
	int head;
 ...
 */
re_threadpool_t* pool_create(int number_of_workers, int queue_size);
 ...
 */
int pool_add_task(re_threadpool_t *pool, void (*function)(void *), void* arg);
